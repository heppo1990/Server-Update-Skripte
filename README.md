...
Der Worker der Windows-Nachinstallationsaufgabe liegt während der offenen Aufgabe unter `%ProgramData%\WindowsUpdateAdm\DeferredUpdates.ps1`. Der Task startet ihn mit einem kurzen `-File`-Aufruf statt eines langen eingebetteten `-EncodedCommand`. Das Mailpasswort wird bei der Task-Erstellung über die bestehende WinRM-Verbindung übertragen, auf dem Zielsystem mit dessen DPAPI geschützt und nur als Chiffretext im Worker hinterlegt. Leserechte für den Worker erhalten nur SYSTEM und die lokalen Administratoren. Nach erfolgreichem Abschluss, einem Fehler oder dem Entfernen der Aufgabe wird die Worker-Datei gelöscht. Der einmalige SYSTEM-Testmailtask verwendet ebenfalls den geschützten Wert.

Der Task-Starthelfer protokolliert zusätzlich Start- und Laufzeitfehler in `%ProgramData%\WindowsUpdateAdm\DeferredUpdates-TaskRunner.log`. Dieses Protokoll bleibt auch dann erhalten, wenn der Worker vor dem Start seiner eigenen Protokollierung scheitert.
