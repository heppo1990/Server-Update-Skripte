...
Der Update-Check wiederholt eine Remote-SYSTEM-Suche einmal nach 20 Sekunden, wenn sie nur ein leeres Ergebnisobjekt zurückgibt. Leere Zeilen werden verworfen und nicht als Updates gezählt. Bleibt die Antwort leer, meldet der Check das Ergebnis als nicht auswertbar. Die Nachinstallationsaufgabe verarbeitet alle konfigurierten zurückgestellten KBs und Kategorien gemeinsam; eine leere KB-Liste unterdrückt keine Kategorie wie `SQL`, `Exchange` oder andere später konfigurierte Werte. Sie kann sowohl nach dem Neustart mit `DeferredUpdateDelayMinutes` als auch im Wartungsfenster des jeweiligen Ziels starten (`PhysicalRebootTime` für physische Rechner, `VMRebootStartTime` für VMs). Der jeweilige Wartungszeitpunkt bleibt auch ohne erforderlichen Neustart aktiv. Nach der Abschlussmail wird ein von `Get-WURebootStatus` bestätigter Neustart sofort ausgelöst.
