...
function Register-HAReboot {
    ...
    elseif ($IsVirtual -and -not [string]::IsNullOrWhiteSpace($vmStartTime)) { $scheduled = (Get-NextHARebootTime $vmStartTime).AddMinutes($VMRebootIndexStart * $interval) }
    elseif (-not $IsVirtual -and -not [string]::IsNullOrWhiteSpace($physicalTime)) { $scheduled = Get-NextHARebootTime $physicalTime }
